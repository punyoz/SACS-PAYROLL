...
const ADMIN_PAGES = {
  ...
  'adm-config':       'System Configuration',
  'adm-backup':       'Backup & Recovery',
  'adm-profile':      'Profile',
};
...
function adminNav(pageId, navEl) {
  ...
  if (pageId === 'adm-config') {
    loadAdmConfig();
  }

  if (pageId === 'adm-backup') {
    loadAdmBackupStatus();
  }
...
async function saveAdmConfig(section) {
  const fb = document.getElementById('adm-config-feedback');
...
/* ═══════════════════════════════════════
   BACKUP & RECOVERY
   ═══════════════════════════════════════ */
async function loadAdmBackupStatus() {
  ...
  try {
    ...
    if (res.ok) {
      ...
      set('adm-backup-last', now);
      set('adm-backup-db-status', dbOk ? 'Connected' : 'Error');
      set('adm-backup-integrity', dbOk ? 'Healthy' : 'Unknown');

      updateAdmBackupPill('adm-bk-db-meta', 'adm-bk-db-pill', dbOk ? 'Connection verified' : 'Connection error', dbOk ? 'Online' : 'Error', dbOk ? 'sa-pill-online' : 'sa-pill-offline');
      updateAdmBackupPill('adm-bk-att-meta', 'adm-bk-att-pill', dbStatus.attendance_logs === 'ok' ? 'Table available' : 'Table missing', dbStatus.attendance_logs === 'ok' ? 'Active' : 'Missing', dbStatus.attendance_logs === 'ok' ? 'sa-pill-online' : 'sa-pill-warning');
      updateAdmBackupPill('adm-bk-pay-meta', 'adm-bk-pay-pill', dbStatus.payroll_records === 'ok' ? 'Table available' : 'Table missing', dbStatus.payroll_records === 'ok' ? 'Active' : 'Missing', dbStatus.payroll_records === 'ok' ? 'sa-pill-online' : 'sa-pill-warning');
      updateAdmBackupPill('adm-bk-prof-meta', 'adm-bk-prof-pill', `${stats.total_users ?? '—'} accounts`, 'Active', 'sa-pill-online');
      updateAdmBackupPill('adm-bk-leave-meta', 'adm-bk-leave-pill', dbStatus.system_config === 'ok' ? 'Table available' : 'Table missing', dbStatus.system_config === 'ok' ? 'Active' : 'Missing', dbStatus.system_config === 'ok' ? 'sa-pill-online' : 'sa-pill-warning');
    } else {
      set('adm-backup-db-status', 'Error');
      set('adm-backup-integrity', 'Unknown');
      updateAdmBackupPill('adm-bk-db-meta', 'adm-bk-db-pill', 'Could not reach system endpoint', 'Error', 'sa-pill-offline');
    }
  } catch (err) {
    set('adm-backup-db-status', 'Offline');
    updateAdmBackupPill('adm-bk-db-meta', 'adm-bk-db-pill', err.message, 'Error', 'sa-pill-offline');
  }
}

function updateAdmBackupPill(metaId, pillId, metaText, pillLabel, pillClass) {
...
async function exportAdmData(type) {
  const fb = document.getElementById('adm-backup-feedback');
  ...
  try {
    ...
    admDownloadCsv([headers, ...rows], filename);
